// Original work https://github.com/lucavandro
...
class CodiceFiscale {
  static compute(codiceFiscaleObject) {
    ...
    code += this.dateCode(
      codiceFiscaleObject.day,
      codiceFiscaleObject.month,
      codiceFiscaleObject.year,
      codiceFiscaleObject.gender
    );
    code += this.findLocationCode(
      codiceFiscaleObject.birthplace,
      codiceFiscaleObject.birthplaceProvincia
    );
  ...
  static surnameCode(surname) {
    let codeSurname =
      this.estraiConsonanti(surname) + this.estraiVocali(surname) + 'XXX';
  ...
  static findLocationCode(location, areaCode) {
    ...
    const locationCode = this.CODICI_CATASTALI[
      this.normalizeString(areaCode)
    ].reduce((code, locationTuple) => {
      return this.normalizeString(locationTuple[0]) ===
        this.normalizeString(location)
        ? locationTuple[1]
        : code;
    }, undefined);

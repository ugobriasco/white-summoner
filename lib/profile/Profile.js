...
class Profile {
  ...
  setCredentials({ username = "", password = "" }) {
    ...
    if (!c.username) this.credentials.username = username;
    if (!c.password) this.credentials.password = password;
  }

  //set countryCodes
  setCountryCode(nat) {
    const refCountry = CountryList.filter(
  ...
  fetchAddress({
    ...
    if (!a.region_id) this.address.region_id = region_id;
    return;
  }
  ...
  fetchPhoneNumber(phoneNumber = "") {
    ...
    if (!p.mobile_phone) {
      ...
      this.profile.merchant_profile.mobile_phone = phoneNumber;
  ...
  //set vat_id
  fetchVat(vat_id = "") {
    const p = this.profile.merchant_profile;
    if (!p.vat_id) {
      this.profile.merchant_profile.vat_id = vat_id;
    }
    return;
  }
  ...
  fetchMerchantProfile({
    ...
    const m = this.profile.merchant_profile;
    const p = this.profile.personal_profile;

    if (!m.legal_type_id)
      this.profile.merchant_profile.legal_type_id = legal_type_id;
    if (!m.merchant_category_code)
      this.profile.merchant_profile.merchant_category_code = merchant_category_code;
    if (!m.company_name)
      this.profile.merchant_profile.company_name = company_name;
    if (!m.vat_id) this.profile.merchant_profile.vat_id = vat_id;
    if (!m.permanent_certificate_access_code)
      this.profile.merchant_profile.permanent_certificate_access_code = permanent_certificate_access_code;
    if (!m.website) this.profile.merchant_profile.website = website;
    if (!m.nature_and_purpose)
      this.profile.merchant_profile.nature_and_purpose = nature_and_purpose;
    if (!m.mobile_phone)
      this.profile.merchant_profile.mobile_phone = mobile_phone;
    if (!p.mobile_phone) {
  ...
  updateMerchantCathegoryCode(newMCC) {
    if (!newMCC) return;
    this.profile.merchant_profile.merchant_category_code = newMCC;
    console.log(this.profile.merchant_profile);
    return;
  }
}
